...
- [x] faster-whisper統合
- [x] 動画から音声抽出（ffmpeg）
- [x] **word_timestamps=True で文字起こし処理の実装**
- [x] **単語レベルタイムスタンプ付きトランスクリプトの生成**
- [x] トランスクリプトのJSON保存

### Phase 3: 検索機能（word-level対応）

- [x] **トランスクリプト内での単語レベル検索**
- [x] **word.word フィールドでのマッチング**
- [x] 検索結果の表示（単語+コンテキスト）
- [x] 正確なタイムスタンプ情報の抽出

### Phase 4: 動画切り抜き機能（ストリーミング対応）

- [x] **subprocessでffmpegコマンドを直接実行**
- [x] 指定区間での動画トリミング（一時ファイル）
- [x] **FileResponseでのストリーミング配信**
- [x] **一時ファイルの自動クリーンアップ**

### Phase 5: UI/UX改善

- [x] HTMXによるインタラクティブUI
  - [x] フェードイン/スライドイン アニメーション効果
  - [x] スムーズなトランジション効果（swap:300ms）
  - [x] ローディングスピナーの視覚的改善
- [x] リアルタイム進捗表示
  - [x] 文字起こし処理中の進捗バー（パルスアニメーション付き）
  - [x] 処理状況の詳細表示
  - [x] 2秒ごとのポーリング更新
